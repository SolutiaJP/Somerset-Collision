...
namespace SeedApp.DataContracts.Helpers
{
	public static class AuthTokenHelper
	{
		...
		public static String GenerateAuthToken(IUserModel userModel)
		{
			return Encrypt(userModel.GlobalId.ToString());
		}

		public static String UnPackAuthToken(String token, out DateTime timeStampCreated, out DateTime timeStampExpired)
		{
			...
			//TOKENS SHOULD HAVE 3 PARTS: USER KEY, TOKEN CREATED, TOKEN EXPIRATION
			if (credentialParts.Length == 3)
			{
				//0. USER KEY (UNIQUELY IDENTIFY USER)
				...
				//2. TIMESTAMP WHEN TOKEN EXPIRES
				long ticks2;
				long.TryParse(credentialParts[2].Trim(), out ticks2);
				//TODO: DETERMINE AMOUNT OF TIME SINCE TOKEN WAS CREATED TO SEE IF IT'S STILL VALID.
		...
		private static String Encrypt(String globalId)
		{
			...
			//ENCODE GLOBAL ID WITH TIME STAMP (TICKS) SO WE KNOW EXACT TIME USER AUTHENTICATED
			var encodedValue = EncodeTo64(String.Format("{0}:{1}:{2}", globalId, tokenCreated, tokenExpiration));
